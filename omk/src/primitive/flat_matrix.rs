...
pub struct Array2D<const COL: u8, const ROW: u8, Idx, T: IndexByValue<Idx>> {
    backend: T,
    _phantom: PhantomData<Idx>,
}

pub struct Slice2D<Idx, T: IndexByValue<Idx>> {
    backend: T,
    col: u8,
    row: u8,
    _phantom: PhantomData<Idx>,
...
impl<const COL:u8, const ROW: u8, const N:usize> Array2D<COL, ROW, u16, ProgmemRef<BinPackedArray<N>>> {
    pub const fn from_existing(data: ProgmemRef<BinPackedArray<N>>) -> Self{
        if N != (COL as usize * ROW as usize).div_ceil(8) {
            panic!()
        } else {
            Self {
...
where
    ...
    T: IndexByValue<Idx>,
{
    type SliceContainer: IndexByValue<Idx>;
    ...
    #[inline(always)]
    unsafe fn extract_ro_sized_view_unchecked<const COL: u8, const ROW: u8>(
        &self,
        startcol: u8,
        startrow: u8,
    ...
    #[inline(always)]
    unsafe fn extract_ro_unsized_view_unchecked(
        &self,
        startcol: u8,
        startrow: u8,
        numcol: u8,
...
pub trait Container2DMut<Idx, T> : Container2D<Idx, T>
where
    Idx: Copy + From<u8> + From<u16> + Add<Output = Idx> + Mul<Output = Idx>,
    T: IndexByValueMut<Idx>,
    Self::SliceContainer: IndexByValueMut<Idx>
{
...
where
    T: IndexByValue<Idx> + DataStorage,
...
impl<Idx, T, const COL: u8, const ROW: u8> Container2DMut<Idx, T> for Array2D<COL, ROW, Idx, T>
where
...
impl<Idx, T> Container2DMut<Idx, T> for Slice2D<Idx, T>
where
    T: IndexByValueMut<Idx> + DataStorage,
    Idx: Copy + From<u16> + Mul<Output = Idx> + Add<Output = Idx> + From<u8>,
{
...
where
    T: Deref<Target = Backend>,
    Backend: IndexByValue<Idx> + DataStorage,
...
impl<Idx, Backend, T, const COL: u8, const ROW: u8> Container2DMut<Idx, Backend>
    for SizedView<COL, ROW, Idx, Backend, T>
where
...
impl<Idx, Backend, T> Container2DMut<Idx, Backend> for UnsizedView<Idx, Backend, T>
where
    T: DerefMut<Target = Backend>,
    Backend: IndexByValueMut<Idx> + DataStorage,
    Idx: Copy + Mul<Output = Idx> + Add<Output = Idx> + From<u8> + From<u16>,
