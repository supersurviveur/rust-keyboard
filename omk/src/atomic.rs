...
where
    F: FnOnce(&'a mut OmkKeyboard<User>, &'a mut OmkShared<User>) -> R,
{
    atomic(|| {
        let whole_kb = (keyboard as *mut OmkKeyboard<User>)
            .wrapping_byte_offset(-(offset_of!(OmkMetaHolder<User>, keyboard) as isize))
            .cast::<OmkMetaHolder<User>>();
