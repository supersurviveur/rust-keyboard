...
Building:
- go to examples/rust-keyboard (or you favorite one)
- install the depandancies/configure rust:
  ...
  + Else:
    Install the equivalent of avr-gcc and avr-libc and use these commands to setup rust
      ...
      ```
- run `make build` and get the output in build/rust-keyboard.elf or run `cargo build --release` and get the output in ./target/atmega32u4-none/release/rust-keyboard.elf
